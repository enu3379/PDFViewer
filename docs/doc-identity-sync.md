# 문서 정체성·싱크 가족 설계 — 다운로드/복제 재정의 (v2.1)
...
- 상태: **설계 확정.** Q1~Q5 확정(2026-07-10)에 이어 보완 3건(§4.2 URL 정규화 · §4.3 locator 우선 · §5.1 삭제 수명주기)과
  UX 안내 방침(§10 안내 4건)까지 확정(2026-07-13). **Codex T1~T7 착수 가능. Claude 산출물은 §9.1(D1~D5).**
...
### 4.2 웹 URL 정규화 규칙 (Q6 확정)

웹 locator는 **열 때 사용한 URL**(뷰어 `src`)을 아래 규칙으로 정규화한 문자열이다.

1. scheme·host 소문자화, 기본 포트(`:80`/`:443`) 제거.
2. fragment 제거(`#page=3` 같은 뷰어 상태는 문서 정체성이 아님).
3. **쿼리는 보존한다** — `?file=a.pdf`처럼 문서를 고르는 쿼리가 흔하므로, 지우면 다른 문서가 한 노드에 합쳐진다.
4. 예외로 추적 파라미터(`utm_*`, `fbclid`, `gclid`)만 제거한다. 나머지 쿼리 키는 재정렬하지 않고 원래 순서를 유지한다(서버 의미 불명).
5. 리다이렉트는 따라가지 않는다. 단축링크와 최종 URL로 갈라 열면 노드 둘 — 자동 병합하지 않는다(조용함 우선, 콘텐츠 증거는 후보 안내에만).

같은 문서의 버전 URL(arXiv `v1`/`v2` 등)은 서로 다른 locator = 다른 문서로 둔다. 내용도 실제로 다르므로 §4.3과 일관된다.

### 4.3 같은 locator, 달라진 바이트 (Q7 확정)

locator 일치(1순위)는 바이트 비교보다 우선한다. 같은 URL·경로에서 내용이 갱신되어도(웹 재게시, 로컬 덮어쓰기 저장)
**같은 노드·같은 통**으로 연다 — 사용자의 정체성 감각은 "그 자리의 그 문서"이기 때문. 이때 `contentEvidence`만 새 값으로
갱신한다. 주석 위치가 새 내용과 어긋나는 문제는 앵커 리플로우(§11 백로그)의 몫이며, 정체성 판정은 바꾸지 않는다.
...
### 5.1 삭제·정리 수명주기 (Q8 확정)

**문서 삭제(허브 목록에서 노드 삭제)** — 파괴적 동작이므로 확인 다이얼로그(§10) 후:

1. **통 처리**: 삭제 노드만 쓰던 전용 통이면 통·주석을 함께 삭제한다(사용자의 '삭제' 기대). 다른 활성 멤버와
   공유 중이면 노드만 지우고 통은 보존한다. 다이얼로그 문구가 두 경우를 구분해 알린다(§10).
2. **허브 승계**: 삭제 노드를 `syncHubId`로 참조하는 노드들이 있으면 —
   - 삭제 노드와 통을 공유하던 **활성 멤버**가 남아 있으면 그중 `addedAt`이 가장 오래된 노드를 새 루트로
     승격(`syncHubId=null`)하고, 나머지 활성·미결정 참조를 그 노드로 재지정한다. 통·주석은 그대로 — 연동은 끊기지 않는다.
   - 활성 멤버가 없으면(미결정 참조뿐) 참조 노드들을 `detached`로 승격한다. 이미 전용 통이므로 주석 손실 없음.
3. 삭제 후에도 불변식(§5)이 성립해야 하며, dangling `syncHubId`는 허용하지 않는다.

**정리(GC) — 열기·다운로드 시점의 lazy 스윕, 워처 없음**:

- `pending` DownloadBinding: 30일 경과 또는 `interrupted`면 삭제(완료 승격에 실패한 잔재 대비).
- 노드: **빈 통 + `detached` + 아무도 자신을 허브로 참조하지 않음 + `lastOpenedAt` 90일 경과**를 모두 만족하면
  삭제한다. 주석이 없으므로 재방문 시 새 노드로 열려도 사용자 관점 손실이 없다. **주석이 있는 노드는 자동
  삭제하지 않는다.**
...
- **보완 3건 확정(2026-07-13)**: 웹 URL 정규화 규칙(§4.2), locator 우선·바이트 변경 시 동일 노드(§4.3),
  삭제·정리 수명주기(§5.1).
- **UX 안내 4건은 UI 문구·안내로 해결(은우 확정 2026-07-13)**: 미결정 첫 오픈 1회 힌트 · 싱크 대상 표시 ·
  충돌 다이얼로그 단방향 명시 · 스냅샷 경고 — §10에 반영, 문구·시각 사양은 Claude 산출물(§9.1 D1~D3).

### 쟁점별 확정 기록 (전부 확정 — 구현 전 재검토용)
| Q | 쟁점 | 확정 |
|---|---|---|
...
| Q6 | 웹 URL 정규화 | **확정: §4.2** — host 소문자·기본 포트 제거, fragment 제거, 쿼리 보존(추적 파라미터만 제거, 재정렬 없음), 리다이렉트 비추적. 버전 URL(arXiv v1/v2)은 다른 문서 |
| Q7 | 같은 locator에서 내용 갱신 | **확정: §4.3** — locator 우선, 같은 노드·같은 통 유지. `contentEvidence`만 갱신, 어긋난 앵커는 리플로우 백로그(§11) |
| Q8 | 삭제·정리 수명주기 | **확정: §5.1** — 전용 통은 함께 삭제·공유 통은 보존, 허브 승계는 최고참 활성 멤버, pending 바인딩 30일·빈 detached 노드 90일 lazy 스윕. 주석 있는 노드는 자동 삭제 금지 |
...
- **T2 — 정체성 resolver** (`viewer/pdf-host.ts` docId 제거, 신규 `core/doc-identity.ts`): §4 우선순위,
  웹 locator는 §4.2 정규화 함수로 생성(유닛 테스트 포함).
  **resolver를 `IdentityProvider` 인터페이스 뒤에 둔다**(v1 구현 = B′: 경로/FSA handle+콘텐츠 증거+mtime,
  ...
  fragment·추적 파라미터만 다른 URL은 동일 노드, 쿼리가 다르면 다른 노드(§4.2). 내용이 갱신된 재방문은 같은
  노드를 유지한다(§4.3).
- **T3 — 싱크 가족·상태 머신** (`core/sync.ts`): 허브 멤버십 역조회, 통 union/fork, 두 기준 revision 분기검사,
  다운로드·복제·싱크 버튼·해제·삭제의 전이와 lazy 스윕(§3·§5·§5.1). 수용: §3·§5 표대로 전이, 충돌 시 콜백으로
  다이얼로그 위임, 시나리오 §7 1~6 통과. 삭제 시 허브 승계·통 보존/삭제가 §5.1대로 동작하고 dangling
  `syncHubId` 없이 불변식 유지, pending 바인딩·빈 detached 노드 스윕 동작.
...
- **T6 — UI** (`viewer/main.ts`, `hub/hub.ts`, css): 다운로드 2종·복제·export 액션, 싱크 배지·버튼,
  ...
  §10 안내 4건(첫 오픈 힌트·싱크 대상 표시·충돌 단방향 문구·스냅샷 경고)과 삭제 확인 2종을 D1~D3 사양대로 구현.
- **T7 — 문서 갱신**: progress.md, implementation-plan에 정체성 모델 반영.

의존성: **T1 → T2 → T3 → T6**, T4는 T2 이후 병렬, T5는 T4 이후. 순서: T1 → T2 → T3 → T4 → T5 → T6 → T7.

### 9.1 설계 산출물 태스크 (Claude — 구현과 병행)

> D1~D3은 **T6 착수 전** 완료가 목표(Codex가 T1~T3을 진행하는 동안). D4는 T6 완료 후 QA 직전 전달.

- **D1 — 상태 배지·아이콘 시각 사양**: 연동중/미결정/해제 점·알약 시각 정의('여백' 시안 A 토큰 기준),
  툴바 다운로드 분할 버튼(파일만/메모포함/export)과 싱크 버튼 아이콘 SVG, 허브 목록 배지.
  수용: viewer/hub 공용 CSS 토큰 + 인라인 SVG로 전달, 클래식·소다 펜 테마 배경에서 대비 확인.
- **D2 — UI 문구 정본**: 다운로드 3종의 사용자 언어 액션명, 싱크/해제 레이블, 미결정 첫 오픈 1회 힌트,
  스냅샷 경고, 충돌 다이얼로그 본문(단방향 명시 — "다른 사본의 메모를 가져오려면 그 문서에서 싱크"),
  삭제 확인 2종(전용 통/공유 통), 서명 PDF 경고. 수용: §10에 문구가 그대로 들어가 T6가 복붙 가능.
- **D3 — 플로우 명세**: 싱크 대상 표시(실행 전 대상 문서명 노출 방식), 충돌·삭제 다이얼로그 스펙(버튼·기본
  포커스·취소의 의미), 힌트 노출 규칙(문서당 1회, 조용한 토스트). 수용: T6가 추가 질문 없이 구현 가능.
- **D4 — 수동 QA 가이드** (`docs/w33-qa.md`): §7 시나리오 6종 + 엣지(이동/개명 B′, 동일 바이트 2회 다운로드,
  외부 복사, blob 왕복, v1 초기화, 미래 스키마 중단) 체크리스트, macOS(은우)/Windows(진형) 분리.
  수용: 각 항목에 기대 결과 명시.
- **D5 — 문서 동기화**: T7 리뷰(implementation-plan 정체성 모델 반영 검수), progress.md 갱신.

---

## 10. UI 표면 (안내 4건 확정 — 문구·시각 사양은 D1~D3, 구현은 T6)
...
- **삭제 확인 다이얼로그**: §5.1의 두 경우를 문구로 구분 — 전용 통("메모 n개가 함께 삭제됩니다") /
  공유 통("다른 사본과 공유 중인 메모는 유지됩니다").
- **복제 액션**: 허브 문서 목록 컨텍스트 메뉴.
- **안내 4건 (은우 확정 2026-07-13)** — 메커니즘은 조용히 두고, 아래 네 순간만 사용자에게 말한다:
  1. **미결정 첫 오픈 힌트(문서당 1회)**: 파생 문서를 처음 열 때 조용한 토스트 — "○○의 사본입니다.
     연동하려면 싱크를 누르세요."
  2. **싱크 대상 표시**: 싱크 실행 전 대상 문서명을 노출("○○와 연동합니다") — 자동 선택(최근 편집 1개)을
     무언으로 두지 않는다.
  3. **충돌 단방향 명시**: [덮어쓰기]/[취소]만 제공하는 것은 의도 — 문구에 "다른 사본의 메모를 가져오려면
     그 문서에서 싱크하세요"를 포함.
  4. **스냅샷 경고**: 메모포함·export 파일은 저장 시점 내용 — 다운로드 완료 안내에 포함해 공유 전
     재다운로드를 유도. 구식 자동 감지는 백로그(§11).
